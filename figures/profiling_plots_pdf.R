...
tool_names <- data.frame(
  tool = c("BarCounter",
           "CITE-seq_Count (UMI correction)",
           "CITE-seq_Count NO Correction",
           "Cellranger Count",
           "KITE (no threading)",
           "KITE (8 threads)"),
  Software = c("BarCounter",
               "CITE-seq-Count (+ correction)",
               "CITE-seq-Count (- correction)",
               "Cell Ranger count",
               "KITE (1 thread)",
               "KITE (8 threads)"),
  point_type = c(19, 1, 3, 17, 5, 4)
)

results <- results %>%
  left_join(tool_names)
...
mem2 <- ggplot() +
  ...
  scale_shape_manual(breaks = tool_names$Software, values = tool_names$point_type) +
  scale_x_continuous(breaks = c(16, 24, 32, 48, 64, 80)) +
  scale_y_continuous(limits = c(0, 24), breaks = seq(0,24,4)) +
  labs(title = "Max. Memory (lower is better)",
       x = "N Cells loaded",
...
wall_time2 <- ggplot() +
  ...
  labs(title = "Elapsed Time (lower is better)",
       x = "N Cells loaded",
...
cpu2 <- ggplot() +
  ...
  labs(title = "CPU Load (lower is better)",
       x = "N Cells loaded",
       y = "percent CPU load") +
  theme(legend.position = "none")

all_plots <- plot_grid(mem2, cpu2, wall_time2, 
          rel_widths = c(1,1,1.7),
          ncol = 3)

ggsave("results_joint_plot.pdf",
       width = 7.5,
       height = 2)
