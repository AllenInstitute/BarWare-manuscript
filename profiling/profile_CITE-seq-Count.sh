...
OUTDIR="/mnt/disks/barcode-tender-manuscript/ADT_count_benchmarking/CITE-seq_Count"
...
for W in $WELLS; do
    ...
    # set command
    COMMAND="/shared/apps/miniconda3/bin/CITE-seq-Count -R1 $( IFS=,; echo "${READ1[*]}" ) -R2 $( IFS=,; echo "${READ2[*]}" ) -t $TAGLIST -o ${OUTDIR}/${NAME} -wl ${BARCODES} -cbf 1 -cbl 16 -umif 17 -umil 28 -cells 40000"

    echo "Runing CITE-seq-Count on ${NAME}"
    printf "\n Command: \n\n$COMMAND\n"

    # run and time command
    $( command time -v -o "/mnt/disks/barcode-tender-manuscript/ADT_count_benchmarking/CITE-seq_Count/CITE-seq_Count_${NAME}_profile.txt" $COMMAND )
...
printf "CITE-seq-Count profiling complete\n"
